                ...
                @forelse($all_courses as $data)
                    <div class="col-lg-4 col-md-6">
                        <div class="course-single-grid-item">
                            <div class="thumb">
                                <a href="{{route('frontend.course.single',[optional($data->lang_front)->slug,$data->id])}}">
                                ...
                                <div class="cat">
                                    <a class="bg-{{$a}}" href="{{route('frontend.course.category',[Str::slug(optional(optional($data->category)->lang_front)->title,'-',optional(optional($data->category)->lang_front)->lang),$data->category->id])}}">{{optional(optional($data->category)->lang_front)->title}}</a>
                                </div>
                            </div>
                            <div class="content">
                                ...
                                @endif
                                <h3 class="title"><a href="{{route('frontend.course.single',[optional($data->lang_front)->slug,$data->id])}}">{{Str::words(optional($data->lang_front)->title,6,'..')}}</a></h3>
                                <div class="instructor-wrap"><span>{{__('By')}}</span> <a href="{{route('frontend.course.instructor',[Str::slug(optional($data->instructor)->name),$data->instructor->id])}}">{{optional($data->instructor)->name}}</a></div>
                                <div class="description">
                                    {!! Str::words(strip_tags(optional($data->lang_front)->description),15) !!}
                                </div>
